...
import com.carlife.daijia.DaijiaOrderCancelActivity;
...
public class RescueCurrentOrderDetailActivity extends Activity implements
		OnClickListener {

	private ImageView ivs1, ivs2, ivs3, ivs4, ivs5;
	private Button btn_back, btn_contact;

	private TextView tv_drivername, tv_usercode, tv_ordercount, tv_star,tv_cancel, tv_type,tv_starttime;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.rescuecurrentorderdetail);
		...
		tv_type.setText(order.getRescueType());
		...
		if(Double.parseDouble(order.getPrice())>0){
	...
	private void payOrder() {
		...
		params.put("sign", strTemp);
		FinalHttp fh = new FinalHttp();
		fh.post(Const.API_SERVICES_ADDRESS + "/PayRescueOrder", params,
				new AjaxCallBack<Object>() {
					@Override
					public void onFailure(Throwable t, int errorNo,
							String strMsg) {

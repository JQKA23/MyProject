...
public class RescueOrderPayActivity extends Activity implements OnClickListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.rescueorderpay);
		context = this;
		...
		order=(RescueOrder)bd.getSerializable("order");		
	...
	private void aliPay() {		
		String orderInfo = AlipayHelper.getOrderInfo("救援订单"+order.getOrderNo()+"付款", "订单号："+order.getOrderNo()+",手机："+bindMobile,order.getPricePlan(), outTradeNo, "http://pay.1018.com.cn/Alipay/HicarRescueOrderReceive");
		...
		Runnable payRunnable = new Runnable() {
			...
			public void run() {
				// 构造PayTask 对象
				PayTask alipay = new PayTask(RescueOrderPayActivity.this);
				// 调用支付接口，获取支付结果
	...
	private void createAlipayOrder(){
		...
		params.put("amount", order.getPricePlan());
		params.put("orderNo", order.getOrderNo());
		Map<String, String> map = new HashMap<String, String>();
		map.put(Const.APPKEY, Const.APPKEY_STR);
		map.put("outTradeNo", outTradeNo);
		map.put("amount", order.getPricePlan());
		...
		fh.post(Const.API_SERVICES_ADDRESS + "/CreateRescueAlipayOrder", params,
	...
	private void createWxOrder(){
		...
		FinalHttp fh = new FinalHttp();
		fh.post(Const.API_SERVICES_ADDRESS + "/CreateRescueWxOrder", params,
				new AjaxCallBack<Object>() {
					@Override
					public void onFailure(Throwable t, int errorNo,
							String strMsg) {
   ...
   //生成订单参数
	private String genProductArgs() {		
		...
		try {
          ...
          List<NameValuePair> packageParams = new LinkedList<NameValuePair>();
			...
			packageParams.add(new BasicNameValuePair("body", "HicarResuce"+order.getOrderNo()));
			packageParams.add(new BasicNameValuePair("mch_id", Const.wxMCH_ID));
			packageParams.add(new BasicNameValuePair("nonce_str", nonceStr));
			packageParams.add(new BasicNameValuePair("notify_url", "http://pay.1018.com.cn/WxPay/HicarForRescueOrderReceive"));
			packageParams.add(new BasicNameValuePair("out_trade_no",outTradeNo));
			packageParams.add(new BasicNameValuePair("spbill_create_ip","127.0.0.1"));	
			String total_fee=String.format("%.0f", Double.parseDouble(order.getPricePlan()) * 100);
	...
	public void onClick(View v) {
		...
		case R.id.btn_billpay:
			Uri uri = Uri
					.parse("http://pay.1018.com.cn/Bill/HicarForRescueBack?id="
							+ order.getId() + "&m=" + Double.parseDouble(order.getPricePlan()) * 100);
		...
		case R.id.btn_alipay: //支付宝支付
			outTradeNo=AlipayHelper.getOutTradeNo(order.getOrderNo());
			createAlipayOrder();
			break;
